...
mkValidator :: PubKeyHash -> POSIXTime -> () -> ScriptContext -> Bool -- param, datum, redeemer, context
...
typedValidator p = Scripts.mkTypedValidator @Vesting
    -- $$(PlutusTx.compile [||mkValidator p||]) -- does not work since everything [||p||] is not known at compile
    -- PlutusTx.liftCode  -- a -> CompiledCodeIn a
    ($$(PlutusTx.compile [|| mkValidator ||]) `PlutusTx.applyCode` PlutusTx.liftCode p)
     $$(PlutusTx.compile [|| wrap ||])
  where
    wrap = Scripts.wrapValidator @POSIXTime @() -- datum, redeemer

validator :: PubKeyHash -> Validator -- also takes a param
validator = Scripts.validatorScript . typedValidator -- using f composition
-- validator p = Scripts.validatorScript $ typedValidator p -- in a more explicit way
...
grab = do
    ...
    -- it will only map if isSuitable (laziness approved)
    if Map.null utxos
